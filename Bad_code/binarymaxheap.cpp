...
namespace ip2
{
    const char* MaxHeapException::what() const throw()
    {
    ...
    bool MaxHeap::operator<(const MaxHeap& other) const
    {
        // Check if the current heap is a subset of the other heap
        for (int value : impl->heap)
        {
            // If value from the current heap is not in the other heap, return false
            if (std::find(other.impl->heap.begin(), other.impl->heap.end(), value) == other.impl->heap.end())
            {
                return false;
            }
        }
        return true;  // Return true if all elements from this heap are in the other heap
    }

    bool MaxHeap::operator>(const MaxHeap& other) const
    {
        // Check if the other heap is a subset of the current heap (reverse of operator<)
        for (int value : other.impl->heap)
        {
            // If value from the other heap is not in the current heap, return false
            if (std::find(impl->heap.begin(), impl->heap.end(), value) == impl->heap.end())
            {
                return false;
            }
        }
        return true;  // Return true if all elements from the other heap are in the current heap
    }

    bool MaxHeap::operator<=(const MaxHeap& other) const
    {
        // Return true if the current heap is a subset of the other heap or if they are identical
        return *this < other || *this == other;
    }

    bool MaxHeap::operator>=(const MaxHeap& other) const
    {
        // Return true if the other heap is a subset of the current heap or if they are identical
        return *this > other || *this == other;
    }
